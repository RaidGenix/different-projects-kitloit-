/*
 * ╔══════════════════════════════════════════════════════════════╗
 *   KITOLIT AI DESK — ESP32 (regular DevKit) + ILI9341 2.4" 240×320
 *
 *   Wiring:
 ...
 *     VCC        → 3V3
 *     GND        → GND
 *   (Touch pins T_IRQ/T_DO/T_DIN/T_CS/T_CLK and SD card pins
 *    SD_SCK/SD_MISO/SD_MOSI/SD_CS are unused — leave unconnected)
 *
 *   WiFi: "Kitolit_AI_Desk"  pw: clawd1234  → http://192.168.4.1
 * ╚══════════════════════════════════════════════════════════════╝
 */
...
// ── WiFi ──────────────────────────────────────────────────────
const char* AP_SSID = "Kitolit_AI_Desk";
const char* AP_PASS = "kitolit123";
WebServer server(80);
...
bool     uiActive     = false;  // becomes true after first web command; gates continuous eye loop
...
static const int16_t LOGO_TRIS[][6] PROGMEM = {
  ...
  {120,105,49,146,51,144},{120,105,65,134,65,134},
};
...
void termDrawHeader() {
  ...
  tft.setCursor(TERM_PAD_X, 4); tft.print("kitolit@aidesk terminal");
  tft.drawFastHLine(0, TERM_PAD_Y, DISP_W, C_ORANGE);
}

// Prefix "kitolit:~$ " in green, drawn only when the row has content
...
#define PREFIX_PX 66   // 11 chars × 6px = 66px at textSize 1
...
// Continuous, non-blocking eye animation — runs every loop() tick, keeps
// looping on its own until currentView changes (e.g. user taps another view).
struct EyeFrame   { int16_t ox; bool blink;  uint16_t dur; };
struct SquishFrame{ bool    closed;          uint16_t dur; };

static const EyeFrame NORMAL_EYE_SEQ[] = {
  {-16, false,  80}, {16, false,  80}, {-16, false,  80}, {16, false,  80}, {0, false,  80},
  {0,   true,  100}, {0,  false,  70}, {0,   true,   70}, {0, false, 900},
};
#define NORMAL_EYE_FRAMES (sizeof(NORMAL_EYE_SEQ) / sizeof(NORMAL_EYE_SEQ[0]))

static const SquishFrame SQUISH_EYE_SEQ[] = {
  {false, 160}, {true, 100},
  {false, 160}, {true, 100},
  {false, 160}, {true, 100},
  {false, 900},
};
#define SQUISH_EYE_FRAMES (sizeof(SQUISH_EYE_SEQ) / sizeof(SQUISH_EYE_SEQ[0]))

uint8_t       eyeFrameIdx = 0;
unsigned long eyeNextMs   = 0;

// Call whenever we switch INTO an eye view so the loop starts from frame 0
void resetEyeAnim() {
  eyeFrameIdx = 0;
  eyeNextMs   = millis();
}

// Call every loop() iteration — advances whichever eye animation is active,
// does nothing (no delay) if it isn't time for the next frame yet.
void updateEyeAnimation() {
  if (!uiActive || termMode) return;
  if ((long)(millis() - eyeNextMs) < 0) return;

  if (currentView == VIEW_EYES_NORMAL) {
    const EyeFrame &f = NORMAL_EYE_SEQ[eyeFrameIdx];
    drawNormalEyes(f.ox, f.blink);
    eyeNextMs   = millis() + speedMs(f.dur);
    eyeFrameIdx = (eyeFrameIdx + 1) % NORMAL_EYE_FRAMES;
  } else if (currentView == VIEW_EYES_SQUISH) {
    const SquishFrame &f = SQUISH_EYE_SEQ[eyeFrameIdx];
    drawSquishEyes(f.closed);
    eyeNextMs   = millis() + speedMs(f.dur);
    eyeFrameIdx = (eyeFrameIdx + 1) % SQUISH_EYE_FRAMES;
  }
}
...
<div class="hdr">
  ...
  <div class="sitename">KITOLIT &middot; AI DESK &middot; CONTROLLER</div>
</div>
...
void routeCmd() {
  ...
  uiActive = true;
  switch (c) {
    case 'w': currentView = VIEW_EYES_NORMAL; resetEyeAnim(); break;
    case 's': currentView = VIEW_EYES_SQUISH; resetEyeAnim(); break;
    case 'd':
...
void setup() {
  ...
  SPI.begin(18, -1, 23, TFT_CS);  // SCK=18, MOSI=23 (VSPI)
  tft.begin();
  tft.setSPISpeed(40000000);
  tft.setRotation(3);
  // If colors look inverted/wrong on first boot, try toggling this:
  ...
  tft.setCursor(DISP_W / 2 - 54, DISP_H / 2 - 22); tft.print("Kitolit");
  tft.setCursor(DISP_W / 2 - 54, DISP_H / 2 + 14); tft.print("AI Desk");
  delay(1200);
  ...
  tft.fillRect(0, 0, DISP_W, 4, C_ORANGE);
  tft.setTextColor(C_WHITE);  tft.setTextSize(2);
  tft.setCursor(12, 16);  tft.print("WiFi: Kitolit_AI_Desk");
  tft.setTextColor(C_MUTED);  tft.setTextSize(1);
  tft.setCursor(12, 44);  tft.print("password: kitolit123");
  tft.setTextColor(C_WHITE);  tft.setTextSize(2);
